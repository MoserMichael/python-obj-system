...
>> id(range_iter): 140238185974448 id(range_iter2): 140238185975120
```
...
```
>> type(not_a_generator): <function not_a_generator at 0x7f8bbf4ef0d0>
...
```
>> caller of generator operating system thread_id: 4519497152
>> inspect.getgeneratorstate(fib_gen): GEN_CREATED
>> (generator) fib_generator operating system thread_id: 4519497152
>> (generator) type(fib_gen.gi_frame): <class 'frame'> fib_gen.gi_frame:  <frame at 0x7f8bbf37b9a0, file '<string>', line 10, code fib_generator>
...
- The currently running task stops running, when it is either waiting for the completion of networking IO, waiting for the completion of another concurrent task or calling the asyncio [sleep api](https://docs.python.org/3/library/asyncio-task.html#sleeping). If any one of these events happened, then the event loop is picking another currently suspended task, and running it instead of the prviously currently running task.
